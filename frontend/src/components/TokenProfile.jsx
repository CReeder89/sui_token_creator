...
import { Button, Box, Typography, Grid, CircularProgress, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from "@mui/material";
...
export default function TokenProfile({ tokenInfo, onSnackbar }) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      onSnackbar(`${dialog.action} transaction built! (sign/send not implemented)`, "info");
      setLoading("");
      closeDialog();
    } catch (err) {
  ...
  return (
    <Box>
      <Typography variant="h4">{tokenInfo.name} ({tokenInfo.symbol})</Typography>
      <Typography variant="subtitle1">Decimals: {tokenInfo.decimals}</Typography>
      <Typography variant="subtitle2">Package: {tokenInfo.packageId}</Typography>
      <Grid container spacing={2} sx={{ mt: 2 }}>
        ...
        <Grid item>
          ...
          >
            {loading === "transfer" ? <CircularProgress size={22} /> : "Transfer"}
          </Button>
        </Grid>
      </Grid>
      <Dialog open={dialog.open} onClose={closeDialog}>
        <DialogTitle>{dialog.action && dialog.action.charAt(0).toUpperCase() + dialog.action.slice(1)}</DialogTitle>
        <DialogContent>
          {(dialog.action === "mint" || dialog.action === "burn" || dialog.action === "transfer") && (
            <TextField
              label="Amount"
              name="amount"
              type="number"
